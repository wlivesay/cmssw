...
namespace trklet {
  ...
  LayerEncoding::LayerEncoding(const DataFormats* dataFormats)
      : setup_(dataFormats->setup()),
        ...
        zT_(&dataFormats->format(Variable::zT, Process::tm)),
        layerEncoding_(
  ...
  void LayerEncoding::analyze(int hitpattern,
    ...
    for (int layerIdKF = 0; layerIdKF <= hp.pmEncode(); layerIdKF++) {
      ...
      static constexpr int numBarrelLayerPS = 3;
      static constexpr int numBarrelLayer = 6;
      bool ps = layerId <= numBarrelLayerPS;
      const bool barrel = layerId <= numBarrelLayer;
      if (!barrel) {
        // calc disk id (0 - 4)
        static constexpr int offset = 11;
        const int diskId = layerId - offset;
        // avergae disk z position
        const double z = setup_->stubDiskZ(diskId) * (cot < 0. ? -1. : 1.);
        // smallest stub radii from 2S disks
        double rLimit = setup_->stubDiskR(diskId) - .5 * setup_->cbcLength();
